...
//metodo para mover el carro + adelante, - atras, potencia abs
    void Carro::mover(int izquierda, int derecha){
        ...
        //Si tienes contadores de vuelta aqui agregaras el ajuste para que avance reto 
        derecha=abs(derecha);
    ...
    void Carro::girar(int grados){
        ...
        contaD=0;
        if(grados>0){
            mover(10,-10); //el giro(giro a la derecha) se realiza moviendo ambas llantas una hacia adelante y la otra hacia atras
        }else if(grados<0){
            mover(-10,10);//giro a la izquierda
        }
